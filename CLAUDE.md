...
| `QClassCycleGuardExtension` | 설정 (`configPackage`) |
...
| `InitializerCodeGenerator` | `QClassInitializer.java` 코드 생성 |
| `CyclicQClassResourceWriter` | `META-INF/cyclic-qclasses.txt` 생성 |

## 빌드 파이프라인

```
compileJava → detectQClassCycle → compileQClassInitializer → classes
```
